    ...
    with st.sidebar:
        ...
        for i, (name, info) in enumerate(CASH_OPTIONS.items()):
            if columns[i].button(info["button"], key=name, use_container_width=True):
                selected_ratio = info["ratio"]
                st.session_state.order_amount = virtual_krw * selected_ratio
                st.rerun()

        st.subheader("가상 보유자산")
        st.info(f"{virtual_krw:,.0f} KRW")
        ...
        while not stop_event.is_set():
            for df in stream_candles(
                ...
                if sig == 1 and not have_coin:
                    ...
                    q.put((df.index[-1], "BUY", price, position, macd, signal))
                    ...
                    q.put((df.index[-1], "SELL", price, position, macd, signal))
...
def render_live_table(log_placeholder) -> None:
    ...
    while not q.empty():
        ...
        if event[0] == "EXCEPTION":
            ...
            if len(event) >= 5:
                ts, type, price, position, macd, signal = event[:5]
                # macd, signal 값을 테이블, 로그 등에 활용
            else:
                ts, type, price = event[:2]
                position = macd = signal = None
            st.session_state.orders.append([ts, type, price, position, macd, signal])
        elif event[1] == "LOG":
    ...
    if st.session_state.orders:
        df_orders = pd.DataFrame(
            ...
            columns=["Time", "Type", "Price", "Position", "MACD", "Signal"],
        )
